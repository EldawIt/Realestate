...
function DetailsUser({ params }) {
  ...
  const updateRooms = async (id, rooms) => {
    const { error, data } = await supabase
      .from("department")
      .update({ rooms: rooms })
      .eq("id", id);
  ...
  const updateKitchen = async (id, kitchen) => {
    const { error, data } = await supabase
      .from("department")
      .update({ kitchen: kitchen })
      .eq("id", id);
  };
  const updateTypeDepartment = async (id, TypeDepartment) => {
    const { error, data } = await supabase
      .from("department")
      .update({ TypeDepartment: TypeDepartment })
      .eq("id", id);
  };
        ...
        <Table sx={{ width: "100%" }}>
          <TableHead>
            <TableRow sx={{ width: "100%" }}>
              ...
              <TableCell align="right">عنوان الشقه</TableCell>
              <TableCell align="right">السعر</TableCell>
              <TableCell align="right">عدد الغرف</TableCell>
              <TableCell align="right">عدد المطبخ</TableCell>
              <TableCell align="right">عدد الحمامات</TableCell>
              <TableCell align="right">نوع الشقه</TableCell>
              ...
              return (
                ...
                >
                  ...
                  <TableCell align="right">
                    <textarea
                      type="text"
                      defaultValue={item.kitchen}
                      style={{ width: "50px" }}
                      updateKitchen
                      onChange={(e) => {
                        let newData = e.target.value;
                        updateKitchen(item.id, newData);
                      }}
                    />
                  </TableCell>
                  <TableCell align="right">
                  ...
                  <TableCell align="right">
                    <textarea
                      type="text"
                      defaultValue={item.TypeDepartment}
                      style={{ width: "50px" }}
                      onChange={(e) => {
                        let newData = e.target.value;
                        updateTypeDepartment(item.id, newData);
                      }}
                    />
                  </TableCell>
